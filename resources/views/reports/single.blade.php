...
<script>
    ...
    <style>
        ...
        [data-headings], [data-links], [data-alts]
        { cursor: pointer; }
        ...
        @media only screen and (max-width: 767px) {
            tr.non-mobile { display: none !important; }
        }

        @media only screen and (min-width: 768px) {
            tr.mobile { display: none !important; }
        }
        ...
        #osr_HeadingsLevelFilter .label { cursor: pointer; }
        ...
        #osr_HeadingsTable .item.l6 { display: none; }

        #osr_HeadingsTable.l1 .item.l1 { display:block; }
        #osr_HeadingsTable.l2 .item.l2 { display:block; }
        #osr_HeadingsTable.l3 .item.l3 { display:block; }
        #osr_HeadingsTable.l4 .item.l4 { display:block; }
        #osr_HeadingsTable.l5 .item.l5 { display:block; }
        #osr_HeadingsTable.l6 .item.l6 { display:block; }

        #osr_PhrasesTable thead { background-color: white; position: sticky; top: -10px; }
        .full-screen-report #osr_PhrasesTable thead { top: 44px !important; }

        #osr_PhrasesFilter .label { cursor: pointer; }

        #osr_PhrasesTable tr.yellow,
        #osr_PhrasesTable tr.grey { display: none; }
        #osr_PhrasesTable.planned tr.yellow { display: table-row; }
        #osr_PhrasesTable.blacklisted tr.grey { display: table-row; }

        /*#osr_PhrasesTable tr { vertical-align: top; }*/
        #osr_PhrasesTable tr .link.icon { visibility: hidden; }
        #osr_PhrasesTable tr:hover .link.icon { visibility: visible; }
        #osr_PhrasesTable tr.yellow [data-action="plan"] { visibility: visible; }
        #osr_PhrasesTable tr.yellow [data-action="blacklist"] { display: none; }
        #osr_PhrasesTable tr.grey [data-action="plan"] { display: none; }
        #osr_PhrasesTable tr.grey [data-action="blacklist"] { visibility: visible; }
    ...
    <script>
        ...
        $(() => { new SingleReportPageControl({ 'reportId': {{ $report->id }}, 'status': {{ $report->status }} }); });
    </script>
